...
public:
  AstroSerialNode() : Node("astro_serial_node"), uart_thread_running_(true)
  {
    ...
    if (this->get_parameter("publish_odom").as_bool()) odom_publisher_ = this->create_publisher<nav_msgs::msg::Odometry>("odom", 10);
    if (this->get_parameter("publish_odom_tf").as_bool() && this->get_parameter("publish_odom").as_bool()) odom_tf_broadcaster_ = std::make_unique<tf2_ros::TransformBroadcaster>(*this);
...
private:
  ...
  void publish_odometry()
  {
    ...
    if((this->get_parameter("publish_odom_tf").as_bool())) odom_tf_broadcaster_->sendTransform(odom_trans);
  ...
  void odom_joint_states_publish_callback()
  {
    ...
    if (this->get_parameter("publish_odom").as_bool()) publish_odometry();
  }
